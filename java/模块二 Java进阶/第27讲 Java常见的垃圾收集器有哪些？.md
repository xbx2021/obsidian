...
![](assets/第27讲%20Java常见的垃圾收集器有哪些？/file-20260516113441293.png)
...
![](assets/第27讲%20Java常见的垃圾收集器有哪些？/file-20260516113527147.png)
第三， 类似第二步的过程会发生很多次，直到有对象年龄计数达到阈值，这时候就会发生所谓的晋升（Promotion）过程，如下图所示，超过阈值的对象会被晋升到老年代。这个阈值是可以通过参数指定：
```bash
-XX:MaxTenuringThreshold=<N>
```
![](assets/第27讲%20Java常见的垃圾收集器有哪些？/file-20260516113607591.png)
后面就是老年代 GC，具体取决于选择的 GC 选项，对应不同的算法。下面是一个简单标记 - 整理算法过程示意图，老年代中的无用对象被清除后， GC 会将对象进行整理，以防止内存碎片化。
![](assets/第27讲%20Java常见的垃圾收集器有哪些？/file-20260516113645622.png)
通常我们把**老年代 GC 叫作 Major GC**，将对**整个堆进行的清理叫作 Full GC**，但是这个也没有那么绝对，因为不同的老年代 GC 算法其实表现差异很大，例如 CMS，“concurrent”就体现在清理工作是与工作线程一起并发运行的。
