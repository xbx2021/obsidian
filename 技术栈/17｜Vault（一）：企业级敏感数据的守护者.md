...
- Kubernetes CA证书：CA证书通常位于 `/var/run/secrets/kubernetes.io/serviceaccount/ca.crt`

- Token Reviewer JWT：创建一个服务账户，或者使用现有的服务账户，并获取其Token。例如，创建一个名为 `vault-auth` 的服务账户，代码就是后面这样。
