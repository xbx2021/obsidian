...
- 精确资源评估：在应用上线前进行详细的资源需求评估，以减少上线后因频繁扩缩容引起的应用不稳定，从而间接节约成本。

- 分析云成本构成：我们可以分析EC2实例的费用构成，并识别出闲置和利用率低的实例。

- 资源申请审计：在申请资源时进行成本预估，如果预见到较高的花费，应进一步分析以避免资源过度申请。

- 运行后的资源评估：应用上线后，定期评估资源使用率，并根据实际使用情况调整资源配置。
