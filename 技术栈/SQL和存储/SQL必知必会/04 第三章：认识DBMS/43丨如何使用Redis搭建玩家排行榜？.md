...
- MySQL是如何实现玩家排行榜的？有哪些难题需要解决？

- 如何用Redis模拟10万名玩家数据？Redis里的Lua又是什么？

- Redis如何搭建玩家排行榜？和MySQL相比有什么优势？
...
- numkeys，代表后续参数key的个数。

- key就是我们要操作的键，可以是多个键。我们在Lua脚本中可以直接使用这些key，直接通过`KEYS[1]`、`KEYS[2]`来获取，默认下标是从1开始。

- arg，表示传入到Lua脚本中的参数，就像调用函数传入的参数一样。在Lua脚本中我们可以通过`ARGV[1]`、`ARGV[2]`来进行获取，同样默认下标从1开始。
