...
而并行探索的方式就不一样了。
![](assets/07%20多任务并行探索与流水线编排/file-20260427143827513.png)
...
![](assets/07%20多任务并行探索与流水线编排/file-20260427143827507.png)
# 项目一：并行探索
...
![](assets/07%20多任务并行探索与流水线编排/file-20260427143827502.png)
...
![](assets/07%20多任务并行探索与流水线编排/file-20260427143827496.png)
...
![](assets/07%20多任务并行探索与流水线编排/file-20260427143827487.png)
...
- 任务之间独立吗？→ 并行
- 任务之间有依赖吗？→ 流水线
![](assets/07%20多任务并行探索与流水线编排/file-20260427143827477.png)
...
流水线的核心不是简单分成多个阶段，而是**阶段间的交接契约设计**——信息必须完整传递，不能让下游重复上游的工作。主对话不是旁观者，而是编排者——它负责审查、决策、介入和回退。流水线失败时，回退到分析阶段而非重试执行阶段——这是避免死循环的关键。
