...
好，到这里，我们优惠券平台项目的第一个模块coupon-template-serv就搭建完成了，你可以在本地启动项目并通过Postman发起调用。我已经将Postman API集合上传到了这个[Gitee源码库](https://gitee.com/banxian-yao/geekbang-coupon-center)中的"资源文件"目录下，文件名为"Spring Boot阶段.postman_collection.json"，你可以导入到自己本地的Postman中使用。
