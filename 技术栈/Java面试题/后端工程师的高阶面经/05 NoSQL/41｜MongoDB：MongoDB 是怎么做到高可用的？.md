...
- 平衡器发送 moveChunk 命令到源分片上。

- 源分片执行 moveChunk 命令，这个时候读写块 A 的操作都是源分片来负责的。

- 目标分片创建对应的索引。

- 目标分片开始同步块 A 的数据。

- 当块 A 最后一个文档都同步给目标分片之后，目标分片会启动一个同步过程，把迁移过程中的数据变更也同步过来。

- 整个数据同步完成之后，源分片更新元数据，告知块 A 已经迁移到了目标分片。

- 当源分片上的游标都关闭之后，它就可以删除块 A 了。
