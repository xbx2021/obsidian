...
- Redis 是如何处理过期 key 的？是立刻删除吗？

- 当你读 Redis 从库的时候，有可能读到过期 key 吗？

- Redis 的 RDB 或者 AOF 是如何处理过期 key 的？
...
- key 太多了，一个 key 一个计时器，Redis 承受不住那么大的计时器开销。
...
- 延迟队列的本身开销很大，尤其是在 key 很多的情况下。

- 修改过期时间需要调整延迟队列中各个 key 的顺序。
