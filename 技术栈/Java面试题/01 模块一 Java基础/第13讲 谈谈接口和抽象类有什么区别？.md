![](assets/第13讲%20谈谈接口和抽象类有什么区别？/file-20260607190043034.png)
...
- **依赖反转**（Dependency Inversion），实体应该依赖于抽象而不是实现。也就是说高层次模块，不应该依赖于低层次模块，而是应该基于抽象。实践这一原则是保证产品代码之间适当耦合度的法宝。[面向对象设计的依赖反转原则](../06%20扩展/面向对象设计的依赖反转原则.md)
