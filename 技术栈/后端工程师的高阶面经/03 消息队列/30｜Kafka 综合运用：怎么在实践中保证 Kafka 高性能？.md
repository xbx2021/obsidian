...
- 性能优化，消息队列相关的优化是其中一个重要的点。

- TCP 协议，Linux 上的几个 TCP 参数以及如何调整这些参数来优化中间件性能，这里可以举 Kafka 的例子。

- swap，追求性能的中间件都会尝试优化 vm.swappiness 参数，你可以以 Kafka 为例。

- 从主从同步引申到 Kafka 主从分区同步，还有批量拉数据模型。

- 压缩，Kafka 的压缩功能，以及你实际使用的压缩算法。

- JVM 或者垃圾回收，可以用优化 Kafka 垃圾回收来证明自己的技术实力。
在做好这些准备之后，你就可以开始面试了。
...
- 当你**选择压缩算法**的时候，你需要权衡压缩比和压缩速率，根据需求做选择。

- **操作系统交换区**可以看作"虚拟内存"。如果触发交换，性能就会显著下降。
