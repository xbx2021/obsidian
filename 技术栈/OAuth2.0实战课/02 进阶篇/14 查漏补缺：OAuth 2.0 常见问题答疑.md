...
- OAuth 2.0 不能被直接用来"从事"身份认证协议的"工作"。虽然OAuth2.0的使用要求是在HTTPS的环境下，但这并不能解决JWT令牌对第三方软件"不透明"的问题，还需要进行加密。

- 有了刷新令牌也不能让访问令牌一直有效下去，因为刷新令牌也有有效期。

- ID令牌是对访问令牌的补充，而不是要替代访问令牌。

- PKCE是OAuth 2.0的一个增补协议，主要用来缓解授权码被窃听的安全风险。
