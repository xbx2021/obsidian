...
- 请求凭证：你的应用通过 API 向 Vault 请求访问特定数据库的凭证。

- 接收凭证和租赁信息：Vault 处理这个请求，生成一个数据库用户名和密码，并创建一个租赁。假设租赁时长设置为24小时。

- 使用凭证：应用使用这些凭证来访问数据库，进行数据读写操作。

- 租赁过期：如果24小时后你没有请求续约，Vault 会自动让这些凭证失效，并从数据库中撤销这些凭证，确保它们不会被再次使用。
...
```
其中&nbsp;[lease_id]&nbsp;是你在租赁信息中获得的 Lease ID。你也可以指定续约的时长，前提是这不超过最大租赁时长的限制。
...
- 撤销密钥或凭证：相关的密钥或凭证将被自动从系统中删除，使其无法再被使用。

- 记录日志：Vault 通常会记录相关的撤销操作，以便于审计和监控。
