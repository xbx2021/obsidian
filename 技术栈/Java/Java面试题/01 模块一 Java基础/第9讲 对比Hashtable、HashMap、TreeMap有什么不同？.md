...
- 重写了 hashCode 也要重写 equals[为什么重写了 hashCode 也要重写 equals。](../06%20扩展/为什么重写了%20hashCode%20也要重写%20equals。.md)。
...
这是因为容量和负载系数决定了可用的桶的数量，空桶太多会浪费空间，如果使用的太满则会严重影响操作的性能。极端情况下，假设只有一个桶，那么它就退化成了链表，完全不能提供所谓常数时间的性能。
