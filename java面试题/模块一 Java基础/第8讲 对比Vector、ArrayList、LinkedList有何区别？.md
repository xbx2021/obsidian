![](assets/第8讲%20对比Vector、ArrayList、LinkedList有何区别？/file-20260607190046829.png)
...
![](assets/第8讲%20对比Vector、ArrayList、LinkedList有何区别？/file-20260607190046831.png)
...
| **有序性**       | **无序**              | **插入顺序**（保持添加顺序）  | **自然排序 / 自定义排序**        |
| **是否允许 null** | 允许 1 个 null         | 允许 1 个 null       | **不允许 null**            |
| **去重规则**      | hashCode()+equals() | 同 HashSet         | Comparable/comparator() |
| **性能**        | 增删查最快 O (1)         | 略慢于 HashSet O (1) | 最慢 O (log n)            |
| **适用场景**      | 只需要去重，不关心顺序         | 需要去重 + 保持插入顺序     | 需要去重 + 自动排序             |
