...
![](assets/03%20-%20子代理Sub-Agents/file-20260422135758823.png)
...
![](assets/03%20-%20子代理Sub-Agents/file-20260422135758813.png)
...
当然，也有一些情况并**不适合使用子代理**。
- 如果任务需要频繁来回确认需求、不断调整方向，那子代理这种“派出去干活再回来汇报”的模式反而会拖慢节奏；

- 如果任务的各个阶段高度耦合，每一步都强依赖上一阶段的详细过程，那强行隔离上下文只会增加认知负担；

- 还有非常简单的小任务，启动子代理本身就有开销，直接在主对话中完成，反而更高效。
...
frontmatter 部分（---  之间）定义子代理的元数据和配置，下方的 Markdown 正文就是子代理的系统提示词（system prompt）。
子代理只会收到这段系统提示词和基本环境信息（如工作目录），不会继承主对话的完整系统提示词。

上述文件中出现的以及未出现的 frontmatter 字段详解如下。其中  name  和  description  是必填字段，其余均为可选：
![](assets/03%20-%20子代理Sub-Agents/file-20260422135758809.png)
## description 的设计艺术
...
## model：模型选择与默认值
![](assets/03%20-%20子代理Sub-Agents/file-20260422135758806.png)
## permissionMode：权限模式

permissionMode  控制子代理在执行过程中遇到需要权限的操作时如何处理。**子代理会继承主对话的权限上下文**，但可以通过此字段覆盖行为：
![](assets/03%20-%20子代理Sub-Agents/file-20260422135758804.png)
...
![](assets/03%20-%20子代理Sub-Agents/file-20260422135758802.png)
...
# 子代理的运行模式
子代理可以在前台或后台运行。
![](assets/03%20-%20子代理Sub-Agents/file-20260422135758801.png)
