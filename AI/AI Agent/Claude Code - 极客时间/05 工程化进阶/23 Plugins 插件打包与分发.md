...
一个插件应该解决一类问题。不要创建万能插件，这与软件工程中的单一职责原则是同一个道理：职责越集中，维护越容易，复用越灵活。
```markdown
好的：
- db-tools — 数据库操作
...
从简单开始，逐步添加功能。不要试图在第一个版本就做到完美。每个版本解决一个明确的问题，让用户有时间适应和反馈。
```markdown
v1.0.0: 基础的代码审查命令
v1.1.0: 添加安全扫描代理
...
只请求必要的工具权限。一个只做分析的代理不需要 Write 和 Edit 工具，一个只做查询的 MCP 不需要写入权限。
```markdown
# 只需要读取的代理
tools: Read, Grep, Glob
...
没有文档的插件是没有价值的。你可能觉得代码和配置“不言自明“，但安装你插件的用户不这么想。他们需要知道每个命令怎么用、每个环境变量什么意思、出了问题怎么排查。文档中必须包含安装方法、每个命令的用法和示例、环境变量说明、更新日志；最好包含使用场景、常见问题和贡献指南。

## 版本管理

使用语义化版本（Semantic Versioning），让用户清楚每次更新的影响范围。
![](assets/23%20Plugins%20插件打包与分发/file-20260430163919955.png)
![](assets/23%20Plugins%20插件打包与分发/file-20260430163926947.png)
![](assets/23%20Plugins%20插件打包与分发/file-20260430164028939.png)
